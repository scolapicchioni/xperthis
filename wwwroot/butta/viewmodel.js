...
export default class ViewModel{
    constructor(repository){
        ...
        document.getElementById('save-trip').addEventListener('click', async () => {
            tripform.classList.toggle('invisible')
            try{
                const newTrip = new Trip(newDestination.value,newDescription.value,newDate.value,+newRating.value)
            
                await this.repository.addTrip(newTrip)
                this.updateUI()
            }catch(error){
                console.log(error)
            }
    ...
    async updateUI(){
        ...
        target.innerHTML = ''
        const tripsList = await this.repository.getAllTrips()
